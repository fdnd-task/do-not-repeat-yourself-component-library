...
function handleSnippet(str, snippetBlock, element) {
    ...
    if (element !== undefined) {
        ...
        if ((_b = snippetBlock.context) === null || _b === void 0 ? void 0 : _b.typeAnnotation) {
            typeAnnotation = surroundWithIgnoreComments(`: import('svelte').Snippet<${str.original.slice(snippetBlock.context.typeAnnotation.start, snippetBlock.context.typeAnnotation.end)}>`);
        }
        const transforms = [
            'var ',
    ...
    handle$propsRune(node) {
        ...
        if (((_a = node.initializer.typeArguments) === null || _a === void 0 ? void 0 : _a.length) > 0) {
            ...
            if (!comments && internalHelpers.isKitRouteFile(this.basename)) {
                const kitType = `{ data: import('./$types.js').${this.basename.includes('layout') ? 'LayoutData' : 'PageData'}, form: import('./$types.js').ActionData }`;
                if (this.isTsFile) {
...
function svelte2tsx(svelte, options = { parse: compiler.parse }) {
    ...
    let { htmlAst, moduleScriptTag, scriptTag, rootSnippets, slots, uses$$props, uses$$slots, uses$$restProps, events, componentDocumentation, resolvedStores, usesAccessors } = processSvelteTemplate(str, options.parse || compiler.parse, options);
